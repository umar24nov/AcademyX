...
| 19 | **Mobile drawer refinement:** both phone hamburgers now open a 1/3-width drawer (landing `side="right"`, dashboard `side="left"`) with the built-in X close button. Key subtlety fixed: `Sidebar`'s `<aside>` is `position:fixed`, so `w-full` = viewport width, NOT the sheet panel width — the mobile variant now uses `relative flex h-full w-full` so it stays inside the 1/3 panel (otherwise content bleeds past the drawer edge). Mobile variant also hides the "AcademyX"/subtitle header, keeps the icon mark only, truncates nav labels, and drops the "Need more seats?" copy to fit 1/3 width. **Removed `MobileBottomNav` entirely** (redundant with the hamburger drawer); deleted `mobile-bottom-nav.tsx` | pending |
